...
class OntologyScheduler:
    ...
    def _load_config(self, config_file: str = None) -> dict:
        ...
        default_config = {
            ...
            "output_dirs": {"reports": "automation/reports", "logs": "automation/logs"},
        }
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if report_file.exists():
                    ...
                    high_priority_changes = report_data.get("changes_by_priority", {}).get(
                        "high", 0
                    )

                    if (
                        high_priority_changes > 0
                        and self.config["thresholds"]["high_priority_notify"]
                    ):
    ...
    def monthly_quality_check(self):
        ...
        try:
            ...
            test_result = subprocess.run(
                test_cmd, cwd=self.project_root, capture_output=True, text=True, timeout=1800
            )

            # Run performance tests specifically
            perf_cmd = ["make", "test-performance"]
            perf_result = subprocess.run(
                perf_cmd, cwd=self.project_root, capture_output=True, text=True, timeout=600
            )
            ...
            if (
                not results["comprehensive_tests"]["passed"]
                or not results["performance_tests"]["passed"]
            ):
    ...
    def _send_notification(self, subject: str, message: str, attachment: str = None):
        ...
        try:
            ...
            if attachment and Path(attachment).exists():
                ...
                part.add_header(
                    "Content-Disposition", f"attachment; filename= {Path(attachment).name}"
                )
                msg.attach(part)
    ...
    def setup_schedule(self):
        ...
        # Daily monitoring
        schedule.every().day.at(self.config["schedules"]["daily_monitoring"]).do(
...
def main():
    ...
    parser.add_argument(
        "--config", default="automation/config.json", help="Configuration file path"
    )
    parser.add_argument(
