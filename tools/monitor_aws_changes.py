...
class AWSChangeMonitor:
    ...
    def monitor_cloudformation_resources(self, compare_with_cache: bool = True) -> dict:
        ...
        # Update cache
        cache_data = {"resources": current_resources, "last_updated": datetime.now().isoformat()}
...
def main():
    ...
    parser = create_base_parser(
        "monitor_aws_changes", "Monitor AWS changes for ontology updates", version="0.4.0"
    )

    parser.add_argument(
        "--source",
        choices=["whats-new", "cloudformation", "api-docs", "all"],
        default="whats-new",
        help="Source to monitor",
    )
    parser.add_argument(
        "--days", type=int, default=7, help="Number of days to look back (for whats-new)"
    )
    parser.add_argument("--services", nargs="+", help="Specific services to monitor (for api-docs)")
    parser.add_argument("--output", help="Output file for detailed report")
    parser.add_argument(
        "--compare", action="store_true", help="Compare with cached data (for cloudformation)"
    )

    args = parser.parse_args()

    # Set up logging
    setup_tool_logging("monitor_aws_changes", args.verbose)
    ...
    try:
        ...
        if args.source in ["cloudformation", "all"]:
            ...
            if cf_result.get("new_resources"):
                cf_change = {
                    "title": "New CloudFormation Resources Detected",
