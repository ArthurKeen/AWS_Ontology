...
class BaseOntologyTest(unittest.TestCase):
    ...
    def assertTripleExists(self, subject, predicate, obj, graph=None):
        ...
        if graph is None:
            graph = self.graph
        self.assertTrue(
            (subject, predicate, obj) in graph, f"Triple not found: ({subject}, {predicate}, {obj})"
        )
    ...
    def assertPropertyExists(self, property_uri, graph=None):
        ...
        self.assertTrue(is_object_prop or is_data_prop, f"Property not found: {property_uri}")
    ...
    def get_class_instances(self, class_uri, graph=None):
        ...
        if graph is None:
            graph = (
                self.combined_graph
                if hasattr(self, "combined_graph")
                else self.load_combined_graph()
            )
